CREATE OR ALTER PROCEDURE dbo.spXML_ProcesarLecturasMedidor
	@Xml XML,
	@FechaOperacion DATE,
    ...
    BEGIN TRY
        ...
        ;WITH LecturasTipo1 AS (
            SELECT
				...
				END AS ConsumoDiff
            FROM @Lecturas AS L
            JOIN dbo.Propiedad AS P ON P.ID = L.IDPropiedad
            WHERE L.TipoMov = 1
        )
        ...
        JOIN LecturasTipo1 AS L ON P.ID = L.IDPropiedad;
        ...
        INTO dbo.MovConsumo(
            ...
            IDPropiedad)
        FROM dbo.Propiedad AS P
        JOIN LecturasTipo2 AS L ON P.ID = L.IDPropiedad;
